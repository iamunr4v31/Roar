...
_WORDS_RE_EN = re.compile(
    rf"([{LATIN_ALPHABET_BASIC}]+(?:[{LATIN_ALPHABET_BASIC}\-']*[{LATIN_ALPHABET_BASIC}]+)*)|(\|[^|]*\|)|([^{LATIN_ALPHABET_BASIC}|]+)"
)
_WORDS_RE_ANY_LOCALE = re.compile(
    rf"([{LATIN_CHARS_ALL}]+(?:[{LATIN_CHARS_ALL}\-']*[{LATIN_CHARS_ALL}]+)*)|(\|[^|]*\|)|([^{LATIN_CHARS_ALL}|]+)"
)
...
def english_text_preprocessing(text, lower=True):
    ...
    text = "".join(
        char if char not in SYNOGLYPH2ASCII else SYNOGLYPH2ASCII[char] for char in text
    )
...
def any_locale_text_preprocessing(text: str) -> str:
    ...
    for c in normalize_unicode_text(text):
        if c in [
            "’"
        ]:  # right single quotation mark (U+2019, decimal 8217) as an apostrophe
            res.append("'")
        else:
            res.append(c)
...
def normalize_unicode_text(text: str) -> str:
    """
    TODO Apply NFC form may be too aggressive since it would ignore some accented characters that do not exist
...
def _word_tokenize(
    words: List[Tuple[str, str, str]], is_lower: bool = False
